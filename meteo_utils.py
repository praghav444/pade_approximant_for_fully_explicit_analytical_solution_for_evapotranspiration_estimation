...
def calculate_VPD_from_RH(Ta, RH):
    """
    Calculate vapor pressure deficit (VPD) in kPa

    Parameters:
    - Ta: Air temperature in °C (scalar, NumPy array, or pandas Series)
    - RH: Relative humidity in % (same shape as Ta)
...
def calculate_RH_from_VPD(Ta, VPD):
    """
    Calculate relative humidity (RH) in percent from air temperature and VPD.

    Parameters:
    - Ta: Air temperature in °C (scalar, NumPy array, or pandas Series)
    - VPD: Vapor Pressure Deficit in kPa (same shape as Ta)
...
def calc_sp_humidity(Ta_C, VPD_kPa, Pa_kPa):
    """
    Calculate specific humidity (kg/kg) from air temperature, VPD, and air pressure.

    Parameters:
        Ta_C : float or np.array
            Air temperature in degrees Celsius
        VPD_kPa : float or np.array
            Vapor pressure deficit in kilopascals (kPa)
        Pa_Pa : float or np.array
    ...
    # Actual vapor pressure (ea = es - VPD)
    ea = es - VPD_kPa
    ea = np.clip(ea, 0, None)  

    # Specific humidity q = 0.622 * ea / (Pa - 0.378 * ea)
    q = (epsilon * ea) / (Pa_kPa - (1 - epsilon) * ea)
